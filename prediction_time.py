...
start_date = pd.Timestamp('2023-05-24', tz='UTC')
...
# Plot the historical data from May 24th, 2023
...
plt.title('Historical Subscriber Counts from May 24th, 2024')
plt.legend()
plt.grid(True)
...
forecast_range = pd.date_range(start=ucx_latest_date, periods=24 * 60, freq='T', tz='UTC')

ucx_forecasted_subs = [ucx_latest_subs + ucx_avg_growth_per_day * i for i in range(len(forecast_range))]
ucozwej_forecasted_subs = [ucozwej_latest_subs + ucozwej_avg_growth_per_day * i for i in range(len(forecast_range))]
